...
.GOTO_TOP:			sc %looptop%;jmpc;
.IS_A_NEWLINE:		sta %50%;lb 0xa;cmp;
.IS_A_FF:			sta %50%;lb 0xff;cmp;
.IS_A_CR:			sta %50%;lb 0xd;cmp;
.IS_A_Q:			sta %50%;lb 0x51;cmp;
.IS_A_R:			sta %50%;lb 0x52;cmp;
.GET_A_BACK:		lda %50%;
...
//small loop.
	:miniLoopTop:
		...
		//if this is less than 127, then we go to dont manip.
				lb 127;cmp;lb 0;cmp;sc %dont_manip%;jmpifeq;
		:still_above_127:
		//generate negative one, twos complement, and put it in b.
			la 1;compl;lb 1;add;ba;
			lda %50%;
				add;
			sta %50%;
		//jump to still_above if the value is >= 127.
			lb 127;cmp;lb 0;cmp;sc %still_above_127%;jmpifneq;
		:dont_manip:
...
:Lbl_BONUSDUCKS:
	...
	:Bonusducks_looptop:
		...
		.numQs:7
			la 0x51;putchar;
			arx0;lb 1;add;rx0a;
			sc %Bonusducks_looptop%;arx0;lb numQs;cmp;lb 0;cmp;jmpifeq;
		lrx0 %/12%;
		lrx1 %/0%;
		rxidiv;
